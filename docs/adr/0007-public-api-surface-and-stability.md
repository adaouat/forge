...
| `updatecheck` | `Checker.CheckNewer`, `Hinter.Print`, `InstallMethod` + detection; `WhatsNewCommand(WhatsNewConfig)` — a `whatsnew` cobra command rendering the release-note span newer than the running build | [0005](0005-updates-via-package-managers.md), [0012](0012-whatsnew-changelog.md) |
