...
| `updatecheck` | `Checker.CheckNewer`, `Hinter` (`Print`; `PostRun` + `OptOutEnv`/`Skip` — the shared post-run hint hook), `CacheFile(app)`, `InstallMethod` + detection; `WhatsNewCommand(WhatsNewConfig)` — a `whatsnew` cobra command rendering the release-note span newer than the running build (config's `Changelog` supplies an embedded offline fallback) | [0005](0005-updates-via-package-managers.md), [0012](0012-whatsnew-changelog.md) |
